...
class SocketService {
  ...
  async connect(): Promise<void> {
    ...
    try {
     const tokenRaw = await AsyncStorage.getItem('authToken');
     ...
     const socketURL = API_BASE_URL.split('/api')[0]; 
      ...
      auth: {
        ...
        extraHeaders: {
          Authorization: `Bearer ${token}`, // Fallback for compatibility
        },
      ...
      this.socket.on('connect_error', (error) => {
        console.error('❌ Socket connection error happened:', error);
